...
# Step B1.2: Check if this domain already has a certificate
existing_cert=$(sudo certbot certificates | grep -i "$domain")
if [ -n "$existing_cert" ]; then
    echo "Certificate for $domain already exists."
    
    # Step B1.3: If this domain already has a certificate, delete it
